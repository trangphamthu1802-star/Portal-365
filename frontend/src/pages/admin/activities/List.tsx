...
import { useActivities, useDeleteArticle, usePublishActivity } from '../../../hooks/useApi';
import { LoadingSpinner } from '../../../components/Loading';

const ACTIVITIES_SLUGS = [
  { slug: 'hoat-dong-cua-thu-truong', name: 'Hoạt động của Thủ trưởng' },
  { slug: 'hoat-dong-cua-su-doan', name: 'Hoạt động của Sư đoàn' },
  { slug: 'hoat-dong-cua-cac-don-vi', name: 'Hoạt động của các đơn vị' },
];

export default function ActivitiesList() {
  const [activeTab, setActiveTab] = useState(ACTIVITIES_SLUGS[0].slug);
  ...
  const handlePublish = async (id: number) => {
    ...
    try {
      ...
      alert('Chức năng gỡ xuất bản chưa được hỗ trợ cho hoạt động');
    } catch (err) {
  ...
  return (
    <AdminLayout title="Quản lý Hoạt động">
      <div className="space-y-6">
        {/* Header */}
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Quản lý Hoạt động</h1>
            <p className="text-gray-600 mt-1">Quản lý bài viết hoạt động theo 3 nhóm</p>
          </div>
          <Link
